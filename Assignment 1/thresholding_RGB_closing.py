...
path = '/Users/marco/Documents/KU Leuven/Computer Vision/Assignment 1/Generated Clips/clip5_thresh_RGB_closing.mp4'
...
while True:
    ...
    closing_img = closing(thresh, 15)
    
    # compute difference between the images
    diff = cv2.absdiff(closing_img, thresh)
    
    # generate blended image accounting for both operations
    thresh = cv2.cvtColor(thresh, cv2.COLOR_GRAY2BGR)
    diff = cv2.cvtColor(diff, cv2.COLOR_GRAY2BGR)
    diff[np.where((diff==[255, 255, 255]).all(axis=2))] = [255, 0, 0]
    blended = cv2.addWeighted(src1=thresh, alpha=0.8, src2=diff, beta=2, gamma=0)
    
    # display and write the blended image
    cv2.putText(blended, text='Closing morphological operator, kernel_size=15', org=(20, 60), fontFace=font, fontScale=2, color=(255, 255, 255),
