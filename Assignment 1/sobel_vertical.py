...
def apply_sobel_x(img):
    ...
    sobel_x = exposure.rescale_intensity(sobel_x, in_range='image', out_range=(0,255)).clip(0,255).astype(np.uint8)
    return sobel_x
...
path = '/Users/marco/Documents/KU Leuven/Computer Vision/Assignment 1/Generated Clips/clip7_sobel_vertical.mp4'
...
while True:
    ...
    frame = apply_sobel_x(frame)
    frame = (frame*255).astype(np.uint8) # convert pixels to integer to allow saving output
    cv2.putText(frame, text='Sobel vertical edge detection, ksize=3', org=(20, 60), fontFace=font, fontScale=2, color=(255, 255, 255),
        thickness=3, lineType=cv2.LINE_AA)
    cv2.imshow('sobel_x', frame)
    writer.write(frame)
